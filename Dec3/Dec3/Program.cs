...
namespace Dec3
{
    class Program
    {
        ...
        private static Dictionary<(int, int), int> ReadPath(string input)
        {
            ...
            {
                var length = int.Parse(dir.Substring(1));
                for (int i = 0; i < length; i++)
                {
                    position.Item1 += xAxis[dir[0]];
                    position.Item2 += yAxis[dir[0]];
                    stepCount++;
                    if (!path.ContainsKey(position))
                        path.Add(position, stepCount);
                }
            }
